...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, config):
    """Subsample proposals and generates target box refinement and class_ids for each.
...
def compute_mrcnn_bbox_loss(target_bbox, target_class_ids, pred_bbox):
    ...
    if torch.nonzero(target_class_ids).size()[0]:
...
def load_image_gt(dataset, config, image_id, augment=False):
    ...
    bbox = custom_dataset.LampPostDataset.extract_bboxes_coco(dataset, image_id)
...
class MaskRCNN(nn.Module):
    ...
    def load_weights(self, filepath, callback=False):
        ...
        if os.path.exists(filepath):
            ...
            if callback:
                state_dict = state_modifier(state_dict)
    ...
    def predict(self, input, mode):
        ...
        if mode == 'inference':
            ...
            # Note that proposal class IDs, and gt_boxes are zero padded.
    ...
    def train_model(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        layers: Allows selecting which layers to train. It can be:
            ...
            - One of these predefined values:
              heads: The RPN and classifier heads of the network
              all: All the layers
        ...
        layer_regex = {
            ...
            "3+": r"(fpn.C3.*)|(fpn.C4.*)|(fpn.C5.*)|(fpn.P5\_.*)|(fpn.P4\_.*)|(fpn.P3\_.*)|(fpn.P2\_.*)|(rpn.*)|("
                  r"classifier.*)",
            "4+": r"(fpn.C4.*)|(fpn.C5.*)|(fpn.P5\_.*)|(fpn.P4\_.*)|(fpn.P3\_.*)|(fpn.P2\_.*)|(rpn.*)|(classifier.*)",
        ...
        for epoch in range(self.epoch + 1, epochs + 1):
            ...
            # Training
            loss, loss_rpn_class, loss_rpn_bbox, loss_mrcnn_class, loss_mrcnn_bbox = self.train_epoch(
    ...
    def valid_epoch(self, datagenerator, steps):
        ...
        for inputs in datagenerator:
            ...
            rpn_match = Variable(rpn_match, volatile=False)
            rpn_bbox = Variable(rpn_bbox, volatile=False)
            gt_class_ids = Variable(gt_class_ids, volatile=False)
            gt_boxes = Variable(gt_boxes, volatile=False)
